...
export default function multiSelect() {
  ...
  function placeDropdown(select) {
    ...
    Util.toggleClass(
      ...
      window.innerWidth < triggerBoundingRect.left + select.dropdown.offsetWidth
    );
    // check if there's enough space up or down
    var moveUp =
      window.innerHeight - triggerBoundingRect.bottom < triggerBoundingRect.top;
  ...
  function keyboardCustomSelect(select, direction, event) {
    ...
    Util.moveFocus(
      select.customOptions[index].getElementsByClassName(
  ...
  function initSelection(select) {
    ...
    select.dropdown.addEventListener("click", function(event) {
      var option = event.target.closest(".js-multi-select__option");
      if (!option || !Util.hasClass(event.target, "js-multi-select__option"))
        return;
      selectOption(select, option);
    });
  }
  ...
  function selectOption(select, option) {
    ...
    select.trigger.getElementsByClassName(
      "js-multi-select__label"
    )[0].innerHTML = triggerLabel[0]; // update trigger label
    Util.toggleClass(
      select.trigger,
  ...
  function getSelectedOptionText(select) {
    ...
    var noSelectionText =
      '<span class="multi-select__term">' + select.noSelectText + "</span>";
    ...
    if (select.selectedOptCounter > select.nMultiSelect) {
      label =
        ...
        select.multiSelectText.replace("{n}", select.selectedOptCounter) +
        "</span>";
      ariaLabel =
        select.multiSelectText.replace("{n}", select.selectedOptCounter) +
        ", " +
    ...
    if (select.insetLabel && select.selectedOptCounter > 0)
      label = noSelectionText + label;
  ...
  function initButtonSelect(select) {
    ...
    var triggerLabel = getSelectedOptionText(select);
    var activeSelectionClass =
      select.selectedOptCounter > 0 ? " multi-select__button--active" : "";
  ...
  function getSelectedOption(select) {
    ...
    if (option)
      return option.getElementsByClassName("js-multi-select__checkbox")[0];
    else
